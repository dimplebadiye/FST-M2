-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/dimple/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/dimple/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/dimple/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/dimple/PigOutput;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/dimple/PigProjectOutput' USING PigStorage('\t');
